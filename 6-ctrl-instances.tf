...
#
#
data "template_file" "ctrl-user-data" {
  template = file("templates/controller-cloud-init")
}

# ===== Creation controller instances =====
resource "openstack_compute_instance_v2" "controller-instances" {
  count = length(var.controller-names)
  name            = format("%s-%s", var.project-name, element(var.controller-names, count.index))
  ...
  user_data       = data.template_file.ctrl-user-data.rendered
...
#  }
  ...
  network {
    ...
    port = openstack_networking_port_v2.ctrlport-deploy[count.index].id
  }

  network {
    name = openstack_networking_network_v2.jacobbaek-network.name
    port = openstack_networking_port_v2.ctrlport-internal[count.index].id
  }

  network {
    name = openstack_networking_network_v2.jacobbaek-network.name
    port = openstack_networking_port_v2.ctrlport-storage[count.index].id
  }

  network {
    name = openstack_networking_network_v2.jacobbaek-network.name
    port = openstack_networking_port_v2.ctrlport-monitor[count.index].id
  }
