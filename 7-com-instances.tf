...
#
#
data "template_file" "compute-user-data" {
  template = file("templates/compute-cloud-init")
}

# ===== Creation compute instances  =====
resource "openstack_compute_instance_v2" "compute-instances" {
...
#  }
  ...
  network {
    ...
    port = openstack_networking_port_v2.comport-deploy[count.index].id
  }

  network {
    name = openstack_networking_network_v2.jacobbaek-network.name
    port = openstack_networking_port_v2.comport-internal[count.index].id
  }

  network {
    name = openstack_networking_network_v2.jacobbaek-network.name
    port = openstack_networking_port_v2.comport-storage[count.index].id
  }

  network {
    name = openstack_networking_network_v2.jacobbaek-network.name
    port = openstack_networking_port_v2.comport-monitor[count.index].id
  }
