pipeline {
    ...
    stages {
        stage('make variables file') {
            steps {
                sh 'sed -i "s/ssh-key_FIXME/${params.PUBKEY}/g" 9-variables.tf'
                sh 'sed -i "s/centos_FIXME/${params.IMAGENAME}/g" 9-variables.tf'
                sh 'sed -i "s/external-network_FIXME/${params.EXTNET-NAME}/g" 9-variables.tf'
                sh 'sed -i "s/internal-network_FIXME/${params.NATNET-NAME}/g" 9-variables.tf'
            }
        }
