...
#
#
data "template_file" "ceph-user-data" {
  template = file("templates/ceph-cloud-init")
}

# ===== Creation ceph instances =====
resource "openstack_compute_instance_v2" "ceph-instances" {
  ...
  user_data       = data.template_file.ceph-user-data.rendered
...
#  }
  ...
  network {
    ...
    port = openstack_networking_port_v2.cephport-deploy[count.index].id
  }

  network {
    name = openstack_networking_network_v2.jacobbaek-network.name
    port = openstack_networking_port_v2.cephport-storage[count.index].id
  }

  network {
    name = openstack_networking_network_v2.jacobbaek-network.name
    port = openstack_networking_port_v2.cephport-monitor[count.index].id
  }
...
#}
#
resource "openstack_compute_volume_attach_v2" "ceph-disk-attach1" {
 count = length(var.ceph-names)
 instance_id = openstack_compute_instance_v2.ceph-instances[count.index].id
 volume_id   = openstack_blockstorage_volume_v3.ceph-disk1[count.index].id
}

resource "openstack_compute_volume_attach_v2" "ceph-disk-attach2" {
 count = length(var.ceph-names)
 instance_id = openstack_compute_instance_v2.ceph-instances[count.index].id
 volume_id   = openstack_blockstorage_volume_v3.ceph-disk2[count.index].id
}

resource "openstack_compute_volume_attach_v2" "ceph-disk-attach3" {
 count = length(var.ceph-names)
 instance_id = openstack_compute_instance_v2.ceph-instances[count.index].id
 volume_id   = openstack_blockstorage_volume_v3.ceph-disk3[count.index].id
}
